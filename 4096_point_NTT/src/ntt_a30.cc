...
void ntt_a30 (input_stream_int32  * restrict s1_in,
	...
	v16int32 x2buff, nullv, x3buff, temp_buff2;
	...
	for (int i =0; i<32; i++){
		...
		x1buff = upd_v(x1buff, 2, readincr_v4(s2_in));

		x1buff = upd_v(x1buff, 6, readincr_v4(s1_in));
		x1buff = upd_v(x1buff, 4, readincr_v4(s2_in));

		x1buff = upd_v(x1buff, 1, readincr_v4(s1_in));
		x1buff = upd_v(x1buff, 3, readincr_v4(s2_in));

		x1buff = upd_v(x1buff, 7, readincr_v4(s1_in));
		x1buff = upd_v(x1buff, 5, readincr_v4(s2_in));

		//aie::vector<int32, 32> v = x1buff;
		//aie::print(v, true ,"vin= ");

		m0 =     srs(lmul8_sym (x1buff, 0, 0x67542310, 8, 24, zbuff, 0, 0x00000000, 0),0);
		m1 = srs(lmul8_antisym (x1buff, 0, 0x67542310, 8, 24, zbuff, 0, 0x00000000, 1),0);
		m2 =     srs(lmul8_sym (x1buff, 0, 0x67542310, 8, 24, zbuff, 0, 0x00000000, 2),0);
		m3 = srs(lmul8_antisym (x1buff, 0, 0x67542310, 8, 24, zbuff, 0, 0x00000000, 3),0);
